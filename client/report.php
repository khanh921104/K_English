...
if ($selected_course_id) {
    // Tổng số video trong khóa học
    $sql_video = "
        SELECT COUNT(DISTINCT v.ma_video) AS tong_video
        FROM buoi_hoc bh
        JOIN video_bai_giang v ON bh.ma_buoi = v.ma_buoi
        WHERE bh.ma_khoa = $selected_course_id
    ";
    $res_video = $mysqli->query($sql_video);
    $row_video = $res_video ? $res_video->fetch_assoc() : ['tong_video'=>0];

    // Tổng số bài tập đã giao
    $sql_baitap = "
        SELECT COUNT(*) AS tong_bai
        FROM bai_tap bt
    ...
    // Tổng số bài tập đã làm và điểm trung bình
    $sql_lambai = "
        SELECT COUNT(*) AS da_lam, AVG(diem) AS diem_tb
...
<head>
    ...
    <style>
        .main-flex { display: flex; gap: 32px; margin: 32px auto; max-width: 1200px; }
        .course-list { flex: 3; background: #fff; border-radius: 12px; padding: 24px 18px; min-width: 220px; max-width: 320px; box-shadow: 0 2px 12px rgba(21,101,192,0.07); height: fit-content; }
        .course-list ul { list-style: none; padding: 0; margin: 0; }
        .course-list li { margin-bottom: 12px; padding: 10px 14px; border-radius: 8px; background-color: #e3f2fd; transition: background 0.3s, color 0.2s; cursor: pointer; }
        .course-list li.selected, .course-list li:hover { background: #1976d2; color: #fff; }
        .course-list li.selected a, .course-list li.selected span, .course-list li:hover a, .course-list li:hover span { color: #fff; }
        .course-list a { color: #1976d2; text-decoration: none; font-weight: 600; transition: color 0.2s; }
        .report-container { flex: 7; background: #fff; border-radius: 12px; box-shadow: 0 4px 24px rgba(21,101,192,0.10); padding: 32px 28px; }
        ...
        <div class="report-container">
            <?php if ($selected_course_id): ?>
                ...
                <div class="stat">
                    ...
                    <!-- <strong>Số video hoàn thành:</strong> <?= $row_video['hoan_thanh'] ?> / <?= $row_video['tong_video'] ?><br> -->
